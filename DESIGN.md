...
- Bottom tab bar on phones, inline header nav on tablet, sidebar from 1440px.
...
- **Card White** (card-white): cards, the header, the tab bar, the sidebar, inputs, outline buttons, sticky action bars.
...
Mobile-first at 375px, with two structural switches: `md` (768px) and `desktop` (1440px).
...
- **Tablet (768px to 1439px):** the tab bar disappears and the destinations join the header inline, centred; gutter 40px.
- **Desktop (≥1440px):** a 240px white sidebar with the wordmark, a vertical nav and the account block at the foot; the header is gone; gutter 56px.

The shell exposes `--tabbar-h` (64px + safe area on phones, 0 from tablet). Sticky action bars (candidate decision, offer form save) sit at `bottom: var(--tabbar-h)` so they rest on the tab bar, not under it; on phones they bleed edge to edge with a top hairline, from tablet they become a rounded card aligned right. The main column reserves `var(--tabbar-h) + 32px` of bottom padding.
...
### Shadow Vocabulary
- **Card** (`0 1px 2px rgb(17 24 39 / 0.04), 0 6px 20px -12px rgb(17 24 39 / 0.22)`): every card, list container and the tablet-width sticky action bar.
- **Raised** (`0 12px 32px -14px rgb(17 24 39 / 0.32)`): toasts and the match screen avatars. Nothing else floats.

### Named Rules
**The Two Heights Rule.** Surfaces are either on the ground (header, tab bar, sidebar: flat, divided by hairlines) or on a card (shadow-card). Raised is reserved for things that arrive: a toast, the match.
...
- **Navigation items** (10px): header and sidebar nav pills.
...
- **Sidebar (desktop):** 240px white rail, 18px icons beside labels, same active treatment; account block (initial avatar on violet wash, name, role) above a hairline at the foot.
...
### Brand
The "rekr." wordmark (violet full stop) in the header and sidebar; the two-circle mark beside it on the splash only.
